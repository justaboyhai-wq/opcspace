# OPCSpace 全业务闭环详细设计 v0.2

> **文档状态：双端原型清单冻结后的业务与系统详细设计讨论基线。** 本文已经按稳定 `MP-* / ADM-*` 编号重建追溯关系；设计仍需业务评审，且不代表功能已经实现。
>
> 编制日期：2026-08-28
>
...
>
> 配套证据：[小程序原型功能全量清单](./OPCSpace-MINI-PROTOTYPE-FUNCTION-INVENTORY-v0.2.md)、[管理端原型功能全量清单](./OPCSpace-ADMIN-PROTOTYPE-FUNCTION-INVENTORY-v0.2.md)、[逐项承接映射](./OPCSpace-BUTTON-TO-BACKOFFICE-MAPPING-v0.2.md)、[PRD v0.1](./OPCSpace-PRD-v0.1.md)、[业务流程 v0.1](./OPCSpace-BUSINESS-FLOWS-v0.1.md)、[待确认事项](./OPCSpace-PENDING-TOPICS.md)
...
| 已确认架构 | 双身份域、模块化单体、管理端/小程序 API 分界、首发和盛大厦、核心表携带 `project_id` |
...
| `opc-facility` | 资产、点位、巡检计划/任务、维保、告警和告警—工单关联 |
...
| `opc-integration` | 微信、支付、发票、短信、门禁、停车、电表和 BA 适配器 |
...
| 主体 | 所属身份域 | 主要权限 |
|---|---|---|
| 平台管理员 | Admin | 项目、系统配置、角色、审计和跨项目治理 |
| 物业客服/前台 | Admin | 报修、投诉、访客、客服和服务受理 |
| 工程/环境/安保人员 | Admin | 被分配任务、现场处理、巡检、核销或异常处置 |
| 财务人员 | Admin | 账单、收款、充值、发票、押金、对账和关账；高风险职责分离 |
| 招商/租赁人员 | Admin | 企业、合同、续退租、装修、承包商和交接 |
| 内容/运营人员 | Admin | 公告、活动、企业服务、Banner、空间和订阅商品 |
...
新增统一补偿台，展示通知、上传处理、支付回调、开票、门禁下发、停车同步、导出等异步任务。每项包含业务对象、外部 ID、当前事实、失败分类、重试历史、下一动作、责任人、截止时间和审计。禁止通过直接改数据库完成补偿。
...
| 工程设施 | 无直接申请入口；通过 MP-01/MP-05/MP-07 产生影响 | ADM-03-01 至 ADM-03-04、ADM-01-04 | 6.15 | 管理端主业务 |
...
完成标准：小程序与管理端读取同一工单和时间线；工程人员不能代用户评价；通知失败不改变工单事实；每次转派保留前后责任人。
...
| 状态 | 车辆 `submitted -> verified/rejected`；月卡 `pending -> active -> expiring -> expired/suspended`；停车单 `open -> priced -> paying -> paid -> exited` |
| 关键异常 | 厂商车牌不同步、重复入场、无入场记录、优惠叠加、支付成功未抬杆、退款、离线出场、月卡变更失败 |

停车厂商、权威车场系统和回执协议未确认前，只设计受理与人工处理，不承诺实时空位、自动抬杆或最终价格。
...
冲突必须在事务内通过锁或数据库约束解决。前端显示空闲不构成预留；确认预约后才能生成签到/门禁凭证。
...
| 子业务 | 小程序 | 管理端 | 关键规则 |
|---|---|---|---|
| 电费账户 | MP-21 余额、用量、阈值、流水 | ADM-04-04、ADM-04-07 | 电表/账务权威源明确；余额由不可变流水计算 |
...
| 空调加时 | 需新增小程序申请入口/记录 | ADM-04-05 确认时长并出账 | 申请时长、确认时长、设备记录和账单可勾稽 |
...
该业务目前主要在管理端，但会通过告警转工单、公告和停机影响小程序用户。

| 层 | 详细设计 |
|---|---|
| 管理端 | ADM-03-01 至 ADM-03-04 维护资产、详情、巡检、维保和告警；ADM-01-04 汇总预警 |
| 后端 | `asset`、`asset_point`、`inspection_plan`、`inspection_task`、`inspection_result`、`maintenance_plan`、`facility_alert` |
| 关联 | 告警可在规则/人工确认后创建工单；维修完成回写关联结果，不删除原告警 |
| 状态 | 巡检任务 `pending -> assigned -> in_progress -> submitted -> reviewed -> completed`；告警 `open -> acknowledged -> processing -> resolved -> closed` |
| 关键异常 | 设备离线、重复告警、漏检、维保商未响应、告警自动派单失败、停机影响通知 |

原型的“事件自动化中心”只能触发受控命令；严重告警自动派单必须有去重窗口、失败重试、死信和人工接管。
...
| S5 | 租赁、装修、工程设施 | 合同/资产唯一主档，审批和现场履约闭环 |
| S6 | 账单、电费、充值、发票、押金 | 渠道回调、对账、退款/红冲和关账通过 |
| S7 | 停车、门禁、设备集成 | 外部状态可区分、可补偿、可对账 |
...
9. 停车系统厂商与权威源；车辆认证、月卡、优惠券、临停减免和异常出场责任。
...
15. 电表、BA、门禁、停车系统的厂商协议、沙箱、回执和数据质量责任。
16. AI 服务商品是否真实售卖；计费、席位、额度、供应商、数据出境和内容安全边界。

这些决策冻结后，应更新 `OPCSpace-PENDING-TOPICS.md`、状态词汇、权限码、OpenAPI、数据库迁移和验收用例，形成可开发的 PRD v0.3。
