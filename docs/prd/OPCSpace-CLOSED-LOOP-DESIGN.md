...
> 文档状态：当前最完整的候选闭环设计附件；未在决策清单关闭的规则仍是候选，不构成正式制度。
>
> 当前内容版本：v0.2 consolidated（文件名不再携带版本号）
>
> 使用规则：当前应按 `FC-*` 逐条评审闭环；本文已经汇总最完整的候选状态机、API、数据表和实施顺序，但只有完成决策关闭并回写主 PRD 的部分才可进入正式需求基线。
>
> 编制日期：2026-08-28
>
...
>
> 当前基线：[OPCSpace PRD](./OPCSpace-PRD.md)、[功能追溯与双端联动](./OPCSpace-FUNCTION-TRACEABILITY.md)。未决规则统一进入[待决策事项](./OPCSpace-DECISIONS.md)；冻结原型盘点和历史候选文件已移入[归档](./archive/README.md)。
>
...
所有尚未冻结、会改变字段、状态机、审批链和验收的事项，只在 [OPCSpace-DECISIONS.md](./OPCSpace-DECISIONS.md) 维护，本文件不再复制第二份问题清单。

决策关闭后应同时回写主 PRD、对应 `FC-*` 和本文件的业务线章节；完成闭环评审后，相关章节才能形成可进入技术设计的需求基线。
