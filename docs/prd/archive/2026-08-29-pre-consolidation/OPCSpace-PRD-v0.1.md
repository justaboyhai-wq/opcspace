# OPCSpace 园区服务一体化平台 PRD v0.1（历史快照）

> **文档状态：历史快照，已由 [OPCSpace PRD v0.3](./OPCSpace-PRD-v0.3.md) 取代。** 本文件只用于追溯 2026-08-28 的初稿，不作为当前范围、设计或验收依据。
>
> 文档版本：v0.1（历史需求讨论稿）
>
> 编制日期：2026-08-28
...
> **历史内容隔离说明：** 本节之后仍保留 v0.1 当时的工程设施、巡检、停车硬件和旧角色设想，仅用于版本追溯；这些内容与 v0.3 冲突时均视为失效，不得复制到新需求、排期、原型或验收用例中。当前范围只从 [PRD v0.3](./OPCSpace-PRD-v0.3.md) 和 [功能联动事实集合 v0.3](./OPCSpace-MINI-ADMIN-FUNCTION-LINKAGE-COLLECTION-v0.3.md) 读取。
...
当前工程事实：`prototypes/` 是本地演示证据；`services/backend/` 与 `apps/mini-app/` 已导入若依源码快照，但仍需切换正式候选基线；`apps/admin-web/` 目前只是边界占位。首期业务模块、真实 API、数据库和双端业务闭环尚未实现。
